        ...
        )
        {
            ...
            foreach (var signalDelegate in godotSignalDelegates)
            {
                ...
                foreach (var paramSymbol in invokeMethodSymbol.Parameters)
                {
                    ...
                    var marshalType = invokeMethodData.ParamTypes[paramSymbol.Ordinal];

                    // Generic type parameters can't be implicitly converted to Variant
                    if (marshalType is MarshalType.GenericType
                        or MarshalType.GenericGodotGenericDictionary
                        or MarshalType.GenericGodotGenericArray)
                    {
                        source.Append("global::Godot.Variant.CreateTakingOwnershipOfDisposableValue(");
                        source.AppendManagedToNativeVariantExpr($"@{paramSymbol.Name}",
                            invokeMethodData.ParamTypeSymbols[paramSymbol.Ordinal], marshalType);
                        source.Append(")");
                        continue;
                    }
